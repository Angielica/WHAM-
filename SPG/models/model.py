...
class Model(nn.Module):
    def __init__(
        ...
        self.encoder_embedding = nn.Linear(
        ...
        self.output_layer = nn.Linear(in_features=embed_dim, out_features=out_dim)
    ...
    def infer_m(self, src: torch.Tensor, tgt_len: int) -> torch.Tensor:
        ...
        for i in range(tgt_len):
            tmp = output.clone()
            output[:, i + 1] = self.forward(src, tmp)[:, i]
        return output[:, 1:]
